#### ***CLASE EXPERIENCIAL DE LA ASIGNATURA ENTORNOS DE DESARROLLO***

#### **App para gestión de incidencias**
<pre><font size="4">
  En <b>Python</b>, con <b>Streamlit</b>
  Con <b>base de datos embebida</b> en <b>SQLite</b>
  <b>Arquitectura en Capas</b>
	
  - Carpeta <b>"VERSION_1"</b>, de la app vista en Sesión Experiencial y mejorada: 
    > Carpeta <b>IncidenciaTickets_V1</b>: todos los archivos de la app
  - Carpeta <b>"VERSION_2"</b>, de la app mejorada con interfaz gráfica (Streamlit + CSS): 
    > Carpeta <b>IncidenciaTickets_V2</b>: todos los archivos de la app
  - Carpeta <b>"VERSION_3"</b>, de la app con interfaz gráfica (Streamlit + CSS):
		- Archivos adicionales para compilar un ejecutable.
		- Archivo ejecutable para Windows (.exe) 
    > Carpeta <b>IncidenciaTickets_V3</b>: todos los archivos de la app + los archivos para automatizar la compilación
	> Ejecutable <b>.exe</b> para <b>Windows</b>
  - Documentación: 
	> <b>"UAX_ENTORNOSDEDESARROLLO-Experiencial-ApuntesClase.pdf"</b> con apuntes de la Sesión Experiencial de la Asignatura
	> <b>"UAX_ENTORNOSDEDESARROLLO-Experiencial-AppGestionTickets_paraIMPRIMIR.pdf"</b> con explicación en detalle de la V1
	> <b>"UAX_ENTORNOSDEDESARROLLO-Experiencial-AppGestionTickets_paraIPANTALLA.pdf"</b> con explicación en detalle de la V1
	> <b>"UAX_ENTORNOSDEDESARROLLO-Experiencial-AppGestionTickets-V2yV3.pdf"</b> con explicación en detalle de las V2 y V3
</font></pre>
